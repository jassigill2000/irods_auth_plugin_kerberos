...
def install_os_specific_dependencies_apt():
    irods_python_ci_utilities.install_os_packages(['make', 'libkrb5-dev', 'libssl-dev', 'gcc'])

def install_os_specific_dependencies_yum():
    irods_python_ci_utilities.install_os_packages(['krb5-devel', 'openssl-devel'])

def install_os_specific_dependencies_zypper():
    irods_python_ci_utilities.install_os_packages(['krb5-devel', 'libopenssl-devel'])
...
def main(output_root_directory, irods_packages_root_directory, externals_directory):
    ...
    build_directory = tempfile.mkdtemp(prefix='irods_kerberos_plugin_build_directory')
